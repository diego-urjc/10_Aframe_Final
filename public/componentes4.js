  ...
  AFRAME.registerComponent('destructor', {
    ...
    init: function () {
        ...
        this.el.addEventListener('raycaster-intersection', function (event) {
            ...
            if (explosion && explosion.components.sound) {
  ...
  init: function () {
    ...
    el.addEventListener('obbcollisionstarted', (event) => {
      ...
      if (target.hasAttribute('jugador')) {
        ...
        // Mensaje de gameover
        let end = document.getElementById('gamefont');

        end.setAttribute('visible', true);
        end.setAttribute('text', 'color: #813d9c');
